...
def seg_augmentation(mode='train', aug_num=AUG_NUM, max_workers=4):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for path in paths:
            ...
            futures.append(
                executor.submit(process_image, image_path, mask_path, augmentation_output_dir, base, aug_num))

        for future in tqdm(futures, file=sys.stdout):
            future.result()
...
if __name__ == '__main__':
    ...
    seg_augmentation('train', 4, max_workers=8)
    seg_augmentation('test', 2, max_workers=8)
